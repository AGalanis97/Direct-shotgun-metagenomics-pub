...
# Import files
import_go_files <- list.files(path = "./Figures/Figure_7/gos",pattern = "\\.fa.txt",full.names = T)
...
go_annotations <- all_go_files %>% purrr::reduce(full_join, by = "GO_annotation") %>% dplyr::select(GO_annotation, everything()) %>% mutate_all(~replace(., is.na(.),0))
...
final_heatmap_functional <- pheatmap(assay(Hives_counts_vst_functional)[ordered_counts_functional,],cluster_rows = FALSE, show_rownames = FALSE, clustering_distance_cols = "correlation", cluster_cols = TRUE, show_colnames = T, annotation_col = df, annotation_colors = ann_colors)

ggsave(plot = final_heatmap_functional, filename="./Figures/Figure_3/functional_heatmap.pdf")
...
# By season
get_volcano_plot(Hives_dds_RLE_functional, .contrast = c('Season','July','November'))
...
functional_significant <- plotDiffAbund_functional(Hives_dds_RLE_functional,pathcsv = "./Figures/Figure_3/functional_significantly_heatmap.csv", colNums = c(1:8))
ggsave(plot = functional_significant, filename = "./Figures/Figure_3/functional_significant.pdf", height = 10, width = 12)
