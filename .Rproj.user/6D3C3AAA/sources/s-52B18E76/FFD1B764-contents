...
family_deseq <- DESeq(Hives_dds_RLE_family)
...
# Export 
classification_deseq_export(normalised_reads = Hives_normalised_counts_family, path_filename = "./Figures/Figure_3/normalised_methodseason_family.csv")
...
export_list <- function(lst, FamGenSp, output_path) {
  names(lst) %>% purrr::walk(~write.csv(lst[[.]], paste0(output_path, FamGenSp,"_cluster_", ., ".csv")))
}
...
arranged_genus <- ggarrange(plotlist =  genus_waffles, ncol = 3, common.legend = T)
arranged_species <- ggarrange(plotlist =  species_waffles, ncol = 3, common.legend = T)

ggsave(plot = arranged_family, filename = "./Figures/Figure_3/family_cluster_composition.pdf")
ggsave(plot = arranged_genus, filename = "./Figures/Figure_3/genus_cluster_composition.pdf")
ggsave(plot = arranged_species, filename = "./Figures/Figure_3/species_cluster_composition.pdf")
...
"May","July","November")  c("#5cb300","#ecf547","#8f7c56")

SM DSM #E41A1C" "#377EB8
brewer.pal(2,"Set1")
----------
...
classification_clr <- function(df) {
  taxids <- rownames(df)
  Species <- as.character(getTaxonomy(taxids,desiredTaxa = "genus",'nameNode.sqlite'))
}
