...
kingdom_melt$Sample <- factor(kingdom_melt$Sample, levels = c("SM_H5","SM_H6","SM_H7","SM_H4","DirectSM_H5","DirectSM_H6","DirectSM_H7","DirectSM_H4"))

# Set axis labels
x_axis_labels <- c("SM H5","SM H6","SM H7","SM H4","DirectSM H5","DirectSM H6","DirectSM H7","DirectSM H4")

# Plot the data. Annotate the parts that have more than 20 species.
barplot_species_absolute <- ggplot(kingdom_melt, aes(x=Sample, y= value, fill=variable)) + geom_bar(stat = "identity") + theme_bw() + scale_fill_manual(values=c("#E6AB02","#eb53a1","#1B9E77","#7570B3")) + labs(y = "Number of species", x="", fill="Domain") + theme(text = element_text(size=15),axis.text.x = element_text(angle=45, hjust=1, size = 10)) + scale_x_discrete(labels = x_axis_labels) + geom_text(aes(label=ifelse(value>20,paste0(kingdom_melt_percent5$value,"%"),"")), position=position_stack(vjust=0.5) , colour="white") 
